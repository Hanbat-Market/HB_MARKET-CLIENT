...
struct VerificationNumView: View {
    ...
    var body: some View {
        VStack {
            ...
            AuthButton(buttonAction: {
                if RegexUtils.isValidStudentEmail(email) {
                    ...
                    notValidEmail.toggle()
                }
            }, buttonText: verifyStudentSuccess ? "인증하기" : "인증번호 전송", backGroundColor: email.isEmpty ? CommonStyle.GRAY_COLOR : CommonStyle.MAIN_COLOR)
        ...
        .onReceive(authVM.matchStudentFailed, perform: {
            matchStudentFailed = true
        })
        .onReceive(authVM.matchStudentSuccess, perform: {
            matchStudentSuccess = true
        })
        .alert(isPresented: $notValidEmail, content: {
            Alert(title: Text("이메일 형식이 올바르지 않습니다."), dismissButton: .default(Text("확인")))
        })
        .alert(isPresented: $matchStudentFailed, content: {
            Alert(title: Text("인증 번호가 올바르지 않습니다."), dismissButton: .default(Text("확인")))
        })
        .alert(isPresented: $matchStudentSuccess, content: {
            Alert(title: Text("재학생 인증되었습니다."), dismissButton: .default(Text("확인"), action: {
                self.dismiss()
            }))
        })
        .navigationDestination(isPresented: $verifyStudentSuccess) {
            CompletedView()
        }
        .onAppear {
